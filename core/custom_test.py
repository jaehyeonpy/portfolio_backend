...
def test_request_to_django_with_not_cached_redis():
    ...
    # check the log to see if this works well too.
    print(f'returned:\n{VideoPersonalityFetchView.as_view()(req, "LrYMUPsjtRw").content.decode("utf-8")}')  
...
def test_redis_optimistic_write_lock():
    def request_to_django_with_not_cahced_redis():
        rf = RequestFactory()
        req = rf.get('/LrYMUPsjtRw') 

        prettifed_json = VideoPersonalityFetchView.as_view()(req, 'LrYMUPsjtRw').content.decode('utf-8')

        # check the log to see if this works well too.
        print(f'returned:\n{prettifed_json}')  
        
    initialize_setting()
    threads = [threading.Thread(target=request_to_django_with_not_cahced_redis) for _ in range(2)]
    [t.start() for t in threads]
    [t.join() for t in threads]


# uncomment each function to test each function; each test is independent on each other.
